...
def solve_08q():
    time_matrix = np.array([[6, 3, 12, 9], [2, 1, 4, 3]])
    cost_matrix = np.array([[6, 3, 12, 9], [8, 4, 16, 12]])
    paths = [[0, 1, 3], [0, 2, 3]]
    machines = [0, 1]
    deadline = 19
        ...
        for first in machines:
            for second in machines:
                for third in machines:
                        ...
                        time_dict[(zeroth, first, second, third)] = path_times

    # print(time_dict)
    # print(len(list(time_dict.keys())))
    print("08Q: {}/{}".format(len(list(filter(lambda key: max(time_dict[key]) <= deadline, list(time_dict.keys())))),
    ...
    curr_cost = -1
    pos = 0
    for item in {k: v for k, v in sorted(cost_map.items(), key=lambda item: item[1])}.items():
        if item[1] > curr_cost:
            pos += 1
            curr_cost = item[1]
        print(pos, item)
...
def solve_10q():
    time_matrix = np.array([[6, 3, 12, 9, 6], [2, 1, 4, 3, 2]])
    cost_matrix = np.array([[6, 3, 12, 9, 6], [8, 4, 16, 12, 8]])
    ...
    # print(time_dict)
    # print(len(list(time_dict.keys())))
    print("10Q: {}/{}".format(len(list(filter(lambda key: max(time_dict[key]) <= deadline, list(time_dict.keys())))),
                              len(list(time_dict.keys()))))
    solutions_list = list(filter(lambda key: max(time_dict[key]) <= deadline, list(time_dict.keys())))
    cost_map = defaultdict()
    for sol in solutions_list:
