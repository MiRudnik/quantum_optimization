...
for S in [40]:
    ...
    for P in [6]:  # itertools.chain(range(3,13,3), range(17,35,5), range(45,76,15)):
        ...
        cs = cs_large/4.0
        results_file = open(FILENAME + "before.txt", "a+")
        ...
        after_file = open(FILENAME + "after.txt", "a+")
        solver_greedy = SteepestDescentSolver()
        sampleset_pp = solver_greedy.sample_qubo(Q, initial_states=sampling_result)
        embedding = sampling_result.info['embedding_context']['embedding']
        chain_strength = sampling_result.info['embedding_context']['chain_strength']
        max_chain_len = max(len(chain) for chain in embedding.values())
        params_string = "P={} S={} max_chain_len={} chain_strength={}\n".format(P, S, max_chain_len, chain_strength)
        print(params_string)
        after_file.write(params_string)
        print(str(embedding))
        after_file.write(str(embedding) + "\n")
        list_sampling_result = list(sampleset_pp.data())
        sum = 0
        if check_gaps:
            gaps = 50.0
            min_num = list_sampling_result[0].energy
